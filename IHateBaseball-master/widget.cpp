...
/**
 * @brief Widget::on_pushButton_ReturnToMain1_clicked
 *  from the major league list, return back to the menu
 */
...
void Widget::on_pushButton_DisplayMajorLeague_clicked()
{
    ...
    string ini_text = ui->comboBox_Cali_StartPoint_Major->currentText().toStdString();
    string end_text = ui->comboBox_Destination_Major->currentText().toStdString();


    //assign the starting index and ending index based on the names of the options selected
    //from the combo boxes using the MLB_stadiums string vector from widget.h
    if(ini_text != end_text)
    {
        for(int i = 0; i < MLB_S; i++)
        ...
        //PROC: Fill in the final_path vector that contains the names of the
        //  stadiums that are part of the shortest path
    ...
    }//end if(ini_text != end_text)

    //PROC: Set the total distance in miles travelled to the
    //display label after converting to Qstring
    ui->textBrowser_Total_Major->setText(
                    QString::fromStdString(
                    to_string(final_distance)));

    //PROC: Draw the map onto the widget label
...
void Widget::on_pushButton_DisplayAL_clicked()
{
    ...
    int final_index;
    //get from combo box what are the source and destination stadiums
    string ini_text = ui->comboBox_Cali_StartPoint_AL->currentText().toStdString();
    string end_text = ui->comboBox_Destination_AL->currentText().toStdString();
    ...
    //PROC: Set the total distance in miles travelled to the
    //display label after converting to Qstring
    ui->textBrowser_AL_Total->setText(
                    QString::fromStdString(
                    to_string(final_distance)));
...
/**
 * @brief Widget::on_pushButton_ReturnToMainAL_clicked
 * From the AL list page go back to the previous menu
 */
void Widget::on_pushButton_ReturnToMainAL_clicked()
{
    ui->stackedWidget->setCurrentIndex(1);
}

void Widget::on_pushButton_DisplayNL_clicked()
{
    ui->stackedWidget->setCurrentIndex(9);
    QString fullpath = QApplication::applicationDirPath() + "/Major_League_Baseball_team_locations.png";
    QPixmap pixmap(fullpath);
    ...
    int final_index;
    //get from combo box what are the source and destination stadiums
    string ini_text = ui->comboBox_Cali_StartPoint_NL->currentText().toStdString();
    string end_text = ui->comboBox_Destination_NL->currentText().toStdString();
    ...
    //PROC: Set the total distance in miles travelled to the
    //display label after converting to Qstring
    ui->textBrowser_NL_Total->setText(
                    QString::fromStdString(
                    to_string(final_distance)));
...
void Widget::on_pushButton_ReturnToMain_NL_clicked()
{
    ui->stackedWidget->setCurrentIndex(1);
}

/**
 * @brief Widget::on_pushButton_Generate_clicked            //ANGEL'S IMPLEMENTATION
 */
void Widget::on_pushButton_Generate_clicked()
{
    vector<string> selected;
    if(ui->pushButton_LosAngelesAnaheim->isChecked())
    {
        selected.push_back("");
    }
}
...
/**
 * @brief Widget::on_pushButton_CustomTrip_Restart_clicked
 * sets all buttons as unchecked and visible again
 */
void Widget::on_pushButton_CustomTrip_Restart_clicked()
{
    ui->pushButton_LosAngelesAnaheim->setChecked(false);
    ui->pushButton_LosAngelesAnaheim->setHidden(false);
    ui->pushButton_ComericaPark_8->setHidden(false);
    ui->pushButton_ComericaPark_8->setHidden(false);
    ui->pushButton_FenwayPark_8->setHidden(false);
    ui->pushButton_FenwayPark_8->setHidden(false);
    ui->pushButton_LosAngelesAnaheim->setChecked(false);
    ui->pushButton_LosAngelesAnaheim->setHidden(false);
    ui->pushButton_ComericaPark_8->setChecked(false);
    ui->pushButton_ComericaPark_8->setHidden(false);
    ui->pushButton_FenwayPark_8->setChecked(false);
    ui->pushButton_FenwayPark_8->setHidden(false);
    ui->pushButton_GlobeLifeParkArlington_8->setChecked(false);
    ui->pushButton_GlobeLifeParkArlington_8->setHidden(false);
    ui->pushButton_KauffmanStadium_8->setChecked(false);
    ui->pushButton_KauffmanStadium_8->setHidden(false);
    ui->pushButton_MinuteMadePark_8->setChecked(false);
    ui->pushButton_MinuteMadePark_8->setHidden(false);
    ui->pushButton_OcoColiseum_8->setChecked(false);
    ui->pushButton_OcoColiseum_8->setHidden(false);
    ui->pushButton_OrioleParkCamdenYards_8->setChecked(false);
    ui->pushButton_OrioleParkCamdenYards_8->setHidden(false);
    ui->pushButton_ProgressiveField_8->setChecked(false);
    ui->pushButton_ProgressiveField_8->setHidden(false);
    ui->pushButton_RogersCentre_8->setChecked(false);
    ui->pushButton_RogersCentre_8->setHidden(false);
    ui->pushButton_SafeCoField_8->setChecked(false);
    ui->pushButton_SafeCoField_8->setHidden(false);
    ui->pushButton_TargetField_8->setChecked(false);
    ui->pushButton_TargetField_8->setHidden(false);
    ui->pushButton_TropicanaField_8->setChecked(false);
    ui->pushButton_TropicanaField_8->setHidden(false);
    ui->pushButton_USCellularField_8->setChecked(false);
    ui->pushButton_USCellularField_8->setHidden(false);
    ui->pushButton_YankeeStadium_8->setChecked(false);
    ui->pushButton_YankeeStadium_8->setHidden(false);
    custom_destinations.clear();
}

void Widget::on_pushButton_LosAngelesAnaheim_clicked()
{
    ui->pushButton_LosAngelesAnaheim->setChecked(true);
    ui->pushButton_LosAngelesAnaheim->setHidden(true);
    custom_destinations.push_back("Angel Stadium");
    //    "PETCO Park",  //0
    //   "Dodger Stadium - ",   //1
    //    "AT&T Park - ", //2
    //                                    "", //3
...
void Widget::on_pushButton_ComericaPark_8_clicked()
{
    ui->pushButton_ComericaPark_8->setChecked(true);
    ui->pushButton_ComericaPark_8->setHidden(true);
    custom_destinations.push_back("Comerica Park");
}

void Widget::on_pushButton_FenwayPark_8_clicked()
{
    ui->pushButton_FenwayPark_8->setChecked(true);
    ui->pushButton_FenwayPark_8->setHidden(true);
    custom_destinations.push_back("Fenway Park");
}

//MISSING FROM DIJKSTRAS AND STADIUM NAMES AND ANYWHERE ELSE IT NEEDS TO BE
//dont implement? Just avoid?
void Widget::on_pushButton_GlobeLifeParkArlington_8_clicked()
{
    ui->pushButton_GlobeLifeParkArlington_8->setChecked(true);
    ui->pushButton_GlobeLifeParkArlington_8->setHidden(true);
//MISSING FROM DIJKSTRAS AND STADIUM NAMES AND ANYWHERE ELSE IT NEEDS TO BE
    //custom_destinations.push_back(" ");
    //dont implement? Just avoid?
}


void Widget::on_pushButton_KauffmanStadium_8_clicked()
{
    ui->pushButton_KauffmanStadium_8->setChecked(true);
    ui->pushButton_KauffmanStadium_8->setHidden(true);
    custom_destinations.push_back("Kauffman Stadium");
}


void Widget::on_pushButton_MinuteMadePark_8_clicked()
{
    ui->pushButton_MinuteMadePark_8->setChecked(true);
    ui->pushButton_MinuteMadePark_8->setHidden(true);
    custom_destinations.push_back("Minute Maid Park");
}


void Widget::on_pushButton_OcoColiseum_8_clicked()
{
    ui->pushButton_OcoColiseum_8->setChecked(true);
    ui->pushButton_OcoColiseum_8->setHidden(true);
    custom_destinations.push_back("O.co Coliseum");
}

//MISSING FROM DIJKSTRAS AND STADIUM NAMES AND ANYWHERE ELSE IT NEEDS TO BE
//dont implement? Just avoid?
void Widget::on_pushButton_OrioleParkCamdenYards_8_clicked()
{
    ui->pushButton_OrioleParkCamdenYards_8->setChecked(true);
    ui->pushButton_OrioleParkCamdenYards_8->setHidden(true);
    //MISSING FROM DIJKSTRAS AND STADIUM NAMES AND ANYWHERE ELSE IT NEEDS TO BE
    //dont implement? Just avoid?
   // custom_destinations.push_back("");
}
...
void Widget::on_pushButton_ProgressiveField_8_clicked()
{
    ui->pushButton_ProgressiveField_8->setChecked(true);
    ui->pushButton_ProgressiveField_8->setHidden(true);
    custom_destinations.push_back("Progressive Field");
}


void Widget::on_pushButton_RogersCentre_8_clicked()
{
    ui->pushButton_RogersCentre_8->setChecked(true);
    ui->pushButton_RogersCentre_8->setHidden(true);
    custom_destinations.push_back("Rogers Center");
}


void Widget::on_pushButton_SafeCoField_8_clicked()
{
    ui->pushButton_SafeCoField_8->setChecked(true);
    ui->pushButton_SafeCoField_8->setHidden(true);
    custom_destinations.push_back("Safeco Field");
}


void Widget::on_pushButton_TargetField_8_clicked()
{
    ui->pushButton_TargetField_8->setChecked(true);
    ui->pushButton_TargetField_8->setHidden(true);
    custom_destinations.push_back("Target Field");
}


void Widget::on_pushButton_TropicanaField_8_clicked()
{
    ui->pushButton_TropicanaField_8->setChecked(true);
    ui->pushButton_TropicanaField_8->setHidden(true);
    custom_destinations.push_back("Tropicana Field");
}

//MISSING FROM DIJKSTRAS AND STADIUM NAMES AND ANYWHERE ELSE IT NEEDS TO BE
//dont implement? Just avoid?
void Widget::on_pushButton_USCellularField_8_clicked()
{
    ui->pushButton_USCellularField_8->setChecked(true);
    ui->pushButton_USCellularField_8->setHidden(true);
    //MISSING FROM DIJKSTRAS AND STADIUM NAMES AND ANYWHERE ELSE IT NEEDS TO BE
    //dont implement? Just avoid?
    custom_destinations.push_back("Comerica Park");
}
...
void Widget::on_pushButton_YankeeStadium_8_clicked()
{
    ui->pushButton_YankeeStadium_8->setChecked(true);
    ui->pushButton_YankeeStadium_8->setHidden(true);
    custom_destinations.push_back("Yankee Stadium");
}

void Widget::on_customTrip1_Next_Button_7_clicked()
{
    ui->stackedWidget->setCurrentIndex(7);
}

void Widget::on_pushButton_PreviousCustomTrip2_8_clicked()
{
    ui->stackedWidget->setCurrentIndex(6);
}

void Widget::on_pushButton_GenerateA_clicked()
{
    ui->stackedWidget->setCurrentIndex(8);
    ...
    ui->label_CustomMap->setPixmap(pixmap);
}
